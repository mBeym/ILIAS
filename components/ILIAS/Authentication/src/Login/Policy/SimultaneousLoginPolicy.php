...
final readonly class SimultaneousLoginPolicy implements PostLoginPolicy
{
    public function __construct(
        private SimultaneousLoginSettings $settings,
        private ActiveSessionProbe $sessions
    ) {
    }

    public function evaluate(LoginSubject $subject): Result
    {
        if (!$this->settings->preventSimultaneousLogins()) {
            return new Result\Ok(null);
        }

        if (!$this->sessions->hasOtherActiveSession(
            $subject->userId(),
            $subject->currentSessionId()
        )) {
            return new Result\Ok(null);
        }
